...
def random_surfer(H, max_iter=1000, tol=1e-9,method = 'gauelim'):
  N = H.shape[0]
  random_surfer_H = []
  for i in H.T:
  ...
  if method == 'gauelim':
    return pagerank_gauelim(random_surfer_H)
  elif method == 'iterative':
    return pagerank_iterative(random_surfer_H, max_iter, tol)
  else:
     raise ValueError("Method should be 'gauelim' or 'iterative'")
...
final_ranks_iter = random_surfer(H,method= 'iterative')[0]

print("Final PageRank values using random surfer algorithm implemented using iterative scheme:")
print(np.round(final_ranks_iter, 5))

final_ranks_gauelim = random_surfer(H,method= 'gauelim')

print("Final PageRank values using random surfer algorithm implemented using dominant eigenvector method:")
